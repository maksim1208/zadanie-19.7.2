...
# вхождение с валидными данными для получения ключа - passed
def test_get_api_key_for_valid_user(email=valid_email, password=valid_password):
    status, result = pet_f.get_api_key(valid_email, valid_password)
    assert status == 200
    assert "key" in result

# вхождение с невалидными данными для получения ключа - passed
def test_get_api_key_for_invalid_user(email=invalid_email, password=invalid_password):
    status, result = pet_f.get_api_key(invalid_email, invalid_password)
    assert status == 403


# вхождение валидным паролем и невалидной почтой для получения ключа - passed
...
# вхождение валидной почтой и невалидным паролем для получения ключа - passed
def test_get_api_key_for_invalid_password(email=valid_email, password=invalid_password):
    status, result = pet_f.get_api_key(valid_email, invalid_password)
    assert status == 403

# вхождение с валидными данными для получения ключа и получение списка животных через ключ
...
def test_post_create_pet_with_novalid_key(name="Каспер", animal_type="Кошка", age=2):
    auth_key = {"key": 'cxNqXqbprjkWNshEM2xEbxOkSJkuCtFW1bLGKHzbiEQHgSBP36'}
...
def test_put_pet_with_valid_data_novalid_key(pet_id='0',name='Мангуст',animal_type='Кот', age=1):
    auth_key = {"key": 'cxNqXqbprjkWNshEM2xEbxOkSJkuCtFW1bLGKHzbiEQHgSBP36'}
